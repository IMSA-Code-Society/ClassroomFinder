...
use std::path::Path;
use webpages::{css_handler, directions, editor, home_page, image, input, save, schedule_handle};
mod path;
...
mod webpages {
    ...
    pub async fn schedule_handle(
        ...
        let schedule = path(&get_schedule(user_input).unwrap());
        HttpResponse::Ok().json(serde_json::json!({ "Schedule Input": schedule }))
    }
